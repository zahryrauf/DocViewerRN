...
class Formatter {
    ...
    formatSingleLineCompileIssue(type, filePathAndLocation, fileName, reason, target, project) {
        ...
        const packageName = [project, target].filter(Boolean).join('/');
        // Choose a color.
        const color = type === 'warning' ? chalk_1.default.yellow : chalk_1.default.red;
        const platform = packageName ? color.bold(`${packageName}:`) : '';
        // Choose a symbol.
        ...
        const relativeAppFile = appFileRoot
            ...
            : sanitizedFilePath;
        // Create the message.
        const title = [symbol, platform, reason.trim()].filter(Boolean).join(' ');
        const results = (0, chalk_1.default) `${title}\n   {gray └─${relativeAppFile}}`;
    ...
    formatCommandPhaseFailed(message, scriptOutput, scriptPhase) {
        const header = scriptPhase ? `Script '${scriptPhase.name}' failed` : message;
        const formattedMessage = chalk_1.default.red(symbols_1.ERROR + ' ' + header);
        const location = scriptPhase?.target
            ? chalk_1.default.gray(`   └─${formatBreadCrumb('', scriptPhase.target, scriptPhase.project)}`)
            : '';
        const contextLines = scriptOutput
            .filter(line => line.trim())
            .map(line => this.formatScriptOutputLine(line));
    ...
    /**
     * Format a single line of script output for display in error context.
     * Parses known patterns (e.g. bash errors) into structured output.
     */
    formatScriptOutputLine(line) {
        // Match: bash: line N: /path/to/file: reason
        // Match: bash: /path/to/file: reason
        const bashPathMatch = line.match(/^bash:\s*(?:line \d+:\s*)?(\/.+?):\s*(.+)$/);
        if (bashPathMatch) {
            const [, filePath, reason] = bashPathMatch;
            const resolved = path.resolve(filePath);
            const display = resolved.startsWith(this.props.projectRoot)
                ? slash(path.relative(this.props.projectRoot, resolved))
                : resolved;
            return chalk_1.default.gray(`${reason}: `) + chalk_1.default.cyan(display);
        }
        // Match: bash: line N: command: reason (no absolute path)
        const bashCmdMatch = line.match(/^bash:\s*(?:line \d+:\s*)(.+?):\s*(.+)$/);
        if (bashCmdMatch) {
            const [, command, reason] = bashCmdMatch;
            return chalk_1.default.gray(`${reason}: `) + chalk_1.default.yellow(command);
        }
    ...
    getBuildSummary() {
        const pendingIssue = this.parser.flushPendingCompileIssue();
        return `${pendingIssue ? pendingIssue + '\n' : ''}\n\u203A ${this.errors.length} error(s), and ${this.warnings.length} warning(s)\n`;
    }
