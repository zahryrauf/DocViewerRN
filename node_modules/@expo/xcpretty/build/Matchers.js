...
exports.Matchers = {
    ...
    Warnings: {
         ...
         * `$3` = reason
         */
        COMPILE_WARNING_MATCHER: /^\s*(\/.+\/(.*):.*:.*):\swarning:\s(.+?(?=\(in)?)(?:\(in target '([^']*)' from project '([^']*)'\))?$/m,
        COMPILE_WARNING_INLINE_MATCHER: /^\s*(\/.+\/(.*):.*:.*):\swarning:\s(.+?(?=\(in)?)(?:\(in target '([^']*)' from project '([^']*)'\))$/m,
        /**
         * @regex Captured groups
         * `$1` = ld prefix
         * `$2` = warning message
    ...
    Errors: {
         ...
         * `$3` = reason
         */
        COMPILE_ERROR_MATCHER: /^\s*(\/.+\/(.*):.*:.*):\s(?:fatal\s)?error:\s(.+?(?=\(in)?)(?:\(in target '([^']*)' from project '([^']*)'\))?$/m,
        /**
         * @regex Captured groups
        ...
        /**
         * @regex Captured groups
         * `$1` = whole error (e.g. "Command PhaseScriptExecution failed with a nonzero exit code")
         */
        COMMAND_PHASE_FAILED_MATCHER: /^(Command .+ failed with a nonzero exit code)$/m,
    },
};
