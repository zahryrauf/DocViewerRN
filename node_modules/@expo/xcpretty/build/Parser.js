...
class Parser {
    static { this.MAX_SCRIPT_OUTPUT_LINES = 10; }
    constructor(formatter) {
        ...
        this.recentUnmatchedLines = [];
        /** Tracks the most recently started script phase for error attribution. */
        this.lastScriptPhase = null;
    }
    parse(text) {
        const errorState = this.updateErrorState(text);
        const withPendingIssue = (result) => [errorState.output, result]
            .filter((value) => typeof value === 'string' && !!value.trim())
            .join('\n');
        if (this.shouldFormatWarning()) {
            return withPendingIssue(this.formatCompileWarning());
        }
        if (this.shouldFormatError()) {
            return withPendingIssue(this.formatCompileError());
        }
        if (errorState.matched) {
        ...
        if (this.shouldFormatUndefinedSymbols()) {
            return withPendingIssue(this.formatUndefinedSymbols());
        }
        if (this.shouldFormatDuplicateSymbols()) {
            return withPendingIssue(this.formatDuplicateSymbols());
        }
        ...
        const result = (0, switchRegex_1.switchRegex)(text, [
            [
            ...
            [
                Matchers_1.Matchers.Errors.COMMAND_PHASE_FAILED_MATCHER,
                ([, $1]) => {
                    const context = [...this.recentUnmatchedLines];
                    const scriptPhase = this.lastScriptPhase;
                    this.recentUnmatchedLines = [];
                    this.lastScriptPhase = null;
                    return formatter.formatCommandPhaseFailed($1, context, scriptPhase);
                },
            ],
            [
            ...
            [
                ...
                ([, $1, $2, $3]) => {
                    this.recentUnmatchedLines = [];
                    const scriptName = unescaped([$1])[0];
                    this.lastScriptPhase = { name: scriptName, target: $2, project: $3 };
                    return formatter.formatPhaseScriptExecution(scriptName, $2, $3);
                },
            ],
            ...
            [
                ...
                () => {
                    ...
                    if (trimmed) {
                        this.recentUnmatchedLines.push(trimmed);
                        if (this.recentUnmatchedLines.length > Parser.MAX_SCRIPT_OUTPUT_LINES) {
                            this.recentUnmatchedLines.shift();
                        }
                    }
        ...
        return withPendingIssue(result);
    }
    flushPendingCompileIssue() {
        if (this.formattingError) {
            return this.formatPendingCompileIssue('error');
        }
        if (this.formattingWarning) {
            return this.formatPendingCompileIssue('warning');
        }
        return '';
    }
    ...
    updateErrorState(text) {
        let output = '';
        const flushIssue = () => {
            output = this.flushPendingCompileIssue();
        };
        const updateIssue = (type, [, $1, $2, $3]) => {
            flushIssue();
            this.currentIssue = {
                reason: $3,
                filePath: $1,
            ...
            this.formattingError = type === 'error';
            this.formattingWarning = type === 'warning';
        };
        ...
        return { matched: results !== 'unmatched', output };
    }
    ...
    formatPendingCompileIssue(type) {
        const issue = { ...this.currentIssue };
        this.resetIssueState();
        return this.formatter.formatSingleLineCompileIssue(type, issue.filePath, issue.fileName, issue.reason);
    }
    resetIssueState() {
        this.currentIssue = {};
