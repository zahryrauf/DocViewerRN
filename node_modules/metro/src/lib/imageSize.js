...
function getImageDimensions(type, content, filePath) {
...
function tryParse(parser, content) {
  try {
...
function readTiffDimension(content, entryOffset, isBigEndian) {
...
function readTiffUInt16(content, offset, isBigEndian) {
  return isBigEndian
    ? content.readUInt16BE(offset)
    : content.readUInt16LE(offset);
}
function readTiffUInt32(content, offset, isBigEndian) {
  return isBigEndian
    ? content.readUInt32BE(offset)
    : content.readUInt32LE(offset);
}
function parseWebp(content) {
  if (
...
function readUInt24LE(content, offset) {
  return (
    content[offset] + content[offset + 1] * 256 + content[offset + 2] * 65536
  );
}
function hasBytes(content, offset, length) {
  return (
...
function readAscii(content, start, end) {
  return content.toString("ascii", start, end);
}
function createInvalidImageError(type, filePath) {
